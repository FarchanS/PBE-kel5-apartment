...
def UpdateData(self):
    ...
    try:
        ...
        query = ("UPDATE owner SET Nama = %s, TempatLahir = %s, TanggalLahir = %s, Kelamin = %s, Telepon = %s WHERE IdOwner = %s")
        cur = con.cursor()
...
def select_data(self):
    try:
        ...
        cur = con.cursor()
        cur.execute("SELECT * FROM owner WHERE IdOwner= %s", [id_owner])
        ...
        if result == ():
            ...
            self.Txt_tempatlahir.setText(result[0][2])
            self.Cmb_kelamin.setCurrentText(result[0][4])
            self.Txt_telepon.setText(result[0][5])
